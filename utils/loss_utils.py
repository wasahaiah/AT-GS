...
def my_cos_loss(output, gt, thrsh=0, weight=1):
    cos = torch.sum(output * gt * weight, 0)
    mask = (cos < np.cos(thrsh)) & (cos != 0)
    return (1 - cos[mask.detach()]).mean()

def l2_loss(network_output, gt):
    return ((network_output - gt) ** 2).mean()

# wrong! this is actually entropy loss, encourage output to have a single mode
...
# bce_loss = bce_loss_func(opacity, mask)
