	...
	float3 p_view, float3 n_view, float fx, float fy, float3 ax0, float3 ax1, float* res)
{
	...
	// x0 on screen space (z==1) in cam coord.
    dir_x0[0] = p_prj.x + 1 / S_fix;
    dir_x0[1] = p_prj.y;
    dir_x0[2] = 1;
	// normalized -> direction of x0
    ...
    // decai: ignore gaussian surfels that are nearly parallel to ray
    ...
    prj_x0 = dir_x0[0] * n_view.x + dir_x0[1] * n_view.y + dir_x0[2] * n_view.z; // cosine of angle between normal and direction of x0
    prj_x1 = dir_x1[0] * n_view.x + dir_x1[1] * n_view.y + dir_x1[2] * n_view.z;
    bool cond_prj = (fabsf(prj_x0 / dir_x0_mod) < thrsh_prj) || (fabsf(prj_x1 / dir_x1_mod) < thrsh_prj); // should remove "/ dir_x0_mod" because dir_x0 is already normalized
    if (cond_prj) return true;
    
    // projected screen unit
    float t_temp, t_x0, t_x1, xu0[3], xu1[3], u0[3], u1[3], xu0_mod;
    t_temp = p_view.x * n_view.x + p_view.y * n_view.y + p_view.z * n_view.z; // distance of cam to plane
    t_x0 = t_temp / prj_x0; // slope distance from cam to ray-plane intersection
    t_x1 = t_temp / prj_x1;
	// xu0 is actually x0~ (tilde) in paper Surface Splatting
...
__forceinline__ __device__ float3 depth_differencing(float2 pix_dif, float* Jinv_u0_u1) {
	// 2D displacement vector in local tangent plane
